...
def estimate(score, *basis):
    """An automated score estimate for a criterion: the rubric's scoring rule
    applied mechanically to extracted facts. `score` is "2"/"1"/"0"/"N/A";
    each `basis` string names the fact that drove it (None entries dropped).
    Only used where the rule is checkable without human judgment."""
    return {"score": score, "basis": [b for b in basis if b]}
